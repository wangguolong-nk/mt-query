...
for i in range(1,len(sys.argv)):
    ...
    if argk=='-dates':
        ...
        deleteFlag=int(argv)
if not dates or not deplydir:
    print 'need PARAMETER: dates and deplydir!'
    sys.exit(1)
sequential_datadir=deplydir+'/work_data/sequential/daily'
vectdatadir=deplydir+'/work_data/search/delta_data'
...
query_sequential_model_path = sequential_datadir + '/query.sequential.model.'+date_name
if os.path.exists(query_sequential_model_path):
    sequential_model_json=readdatafromfile(query_sequential_model_path)
deleteFilepaths=[]
for date in date_arr:
    cur_vectpath=vectdatadir+'/query.w2v.'+date+'.vect'
    cur_seqpath=sequential_datadir+'/query.seq.'+date
