...
downloadSearchDate(){
	...
	do
		...
		if [ ! -s $srcFile ];then
        		local hivesql="hive-client -e 'select uid,page,unix_timestamp(time),mv_result,client_id,q,core_user_result from meipai where controller_p=\"search\" and action_p=\"user_mv\" and logdate==\"$startHour\" and uid is not null and (client_id=="1089857299" or client_id="1089857302") ' -q -o $srcFile"
        		echo $hivesql
        		$hivesql
        		if [ $? != 0 ];then
