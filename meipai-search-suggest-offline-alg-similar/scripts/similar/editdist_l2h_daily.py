...
输出：query.editdist.20170520(删除)，query.editdist.all.delta(更新), query.20170520(删除)
'''
...
class QueryEditDist:
    def __init__(self, outdir, delta_querydir, date, full_querydir,deleteFlag):
        self.outdir = outdir
        self.hqpath = full_querydir + '/query.quality.predict.high'
        # self.mode=mode#1:每天产生的query 2:全量的低质量到高质量 3:全量的高质量到高质量
        self.deleteFlag=deleteFlag
        self.deletepath=None
        self.query_editdist_all_delta_path=full_querydir+'/query.editdist.all'
        # if mode==1:
        if 1==1:
    ...
    def compareAll(self):
        ...
        if self.deletepath:
            import os
            print 'self.deletepath delete:',self.deletepath
...
if __name__=='__main__':
    ...
    for i in range(1,len(sys.argv)):
        ...
        if paramk=='-projdatadir':
            ...
            deleteFlag=int(val)
    if not date:
        print 'MUST NEED parameter: date!'
        sys.exit(2)
    if not projdatadir:
        print 'need PARAMETER: projdatadir!'
        sys.exit(2)
    delta_querydir = projdatadir+'/work_data/days'
    full_querydir = projdatadir+'/work_data'
    outdir = delta_querydir
    qdist=QueryEditDist(outdir=outdir,delta_querydir=delta_querydir,full_querydir=full_querydir,date=date,deleteFlag=deleteFlag)
    compareAll_t0=time.time()
    qdist.compareAll()
    qdist.merge_delta()
    print 'over time=',(time.time()-compareAll_t0)