...
for i in range(1,len(sys.argv)):
    ...
    if argk=='-date':
        ...
        deleteFlag=int(argv)
if not date or not projdatadir:
    print 'need PARAMETER: date and projdatadir!'
    sys.exit(1)
print datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
date_arr=[]
datadir=projdatadir+'/work_data/search/hours'
query2vect_dict={}
deletepaths=[]
for i in range(24):
    hourstr=str(i)
    ...
    queryindex_path=datadir+'/query.'+cur_date+'.index'
    queryvect_path=datadir+'/query.'+cur_date+'.vect'
    if not os.path.exists(queryindex_path) or not os.path.exists(queryvect_path):
        continue
...
if len(query2vect_dict)==0:
    sys.exit(0)
outvectpath=projdatadir+'/work_data/search/train_data/query.vect.'+date
